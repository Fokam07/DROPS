...
export default function AdminOrdersPage() {
  ...
  const [search, setSearch] = useState(""); // 🔍 Recherche locale
  ...
  const handleFilter = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:8000/api/admin/orders/filter?${params.toString()}`,
        { headers }
      );
      setOrders(res.data);
    } catch {
  ...
  const handleDelete = async (id_order) => {
    ...
    try {
      await axios.delete(`http://localhost:8000/api/admin/orders/${id_order}`, { headers });
